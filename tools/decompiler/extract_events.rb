...
def generate_event_file(key, event, output_dir)
  event_name = event.name.split(",")[0].gsub(/[^0-9A-Za-z ]/, "")
  FileUtils.mkdir_p(output_dir)
  event_file = output_dir + "event_#{key} - #{event_name}.rb"
  dumper = RPGDumper.new(name: "", maps: "")
  File.write(event_file, dumper.event(event, 0))
  return event_file
end

def generate_event_files(data_file, mapinfos, output_dir)
  map_index = 0
  ...
  map_data.events.each do |key, event|
    generate_event_file(key, event, output_dir)
  end
end

# generate event page files for all the Map data files in a directory
def extract_event_set(source_dir, output_dir)
  ...
  data_dir.each_child do |data_file|
    if data_file.include? "Map" and not(data_file.include? "Infos")
      ...
      generate_event_files(source_dir + data_file, mapinfo, output_dir)
    end
  end
...
# extract_event_set(src_data, target_dir)
